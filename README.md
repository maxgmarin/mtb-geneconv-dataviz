...
mtb-geneconv-dataviz/
  ...
  TBP22_ReseqAnalysisAndViz/  # per-event igv-reports viewer, target vs control isolates,
                              # TBP22/22CI resequencing validation of 12 candidate events
...
### TBP22_ReseqAnalysisAndViz

A table + iframe viewer for 12 candidate gene-conversion events identified in the
...
GitHub repo; it's generated by scripts kept alongside the original working copy at
`/n/data1/hms/dbmi/farhat/mm774/Projects/Mtb-GeneConv/TBP22_ReseqAnalysisAndViz/` (not
duplicated into this repo -- only the built `index.html`/`events_data.js`/`reports/` are).
...
- **TBP22_ReseqAnalysisAndViz**: regenerate from
  `/n/data1/hms/dbmi/farhat/mm774/Projects/Mtb-GeneConv/TBP22_ReseqAnalysisAndViz/scripts/`
  (`merge_event_tables.py` -> `generate_igv_report.py --all` -> `build_viewer_site.py`),
  then copy `index.html`, `events_data.js`, and `reports/` into this repo's
  `TBP22_ReseqAnalysisAndViz/` -- `scripts/` and `results/` are intentionally not
  duplicated here.
...
to read the underlying CRAM/reference data directly. `Mtb151_GCE_GenomeViz` and
`TBP22_ReseqAnalysisAndViz` have no such dependency -- the former serves its data from
GitHub directly, and the latter's reports embed all track data directly in each HTML.
